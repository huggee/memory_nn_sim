...
def trans(n, vg, vd, bias):
    current = np.zeros([3, n])
    for i in range(2):
        a = 1.4 * np.exp(16.0 * vg[i])
        b = 1.0 - np.exp(-0.005 * vd)
        current[i] = np.matmul(b, a)
    current[2] = current[0] - current[1] + bias
...
for iteration in range(max_itr):
    ...
    ### encode
    hidden_x = trans(n_hidden, vg, x_input, bias_h)
    hidden_y = np.where(hidden_x > 0, 1.0, 0.0)
    # hidden_y = 1.0 / (1.0 + np.exp(-hidden_x))

    ### decode
    output_x = trans(n_visible, np.transpose(vg, (0, 2, 1)), -hidden_y, bias_v)
    output_y = np.where(output_x > 0, 1.0, 0.0)

    d_output = output_y
    # d_output = output_y * -1.0 + 1.0

    hid2_x = trans(n_hidden, vg, output_y, bias_h)
    ...
    grad_bv = x_input - output_y
    grad_bh = hidden_x - hid2_x
